...
export function meta({}: Route.MetaArgs) {
  ...
  return [
    { title: `${t('app.name')} - ${t('auth.register.title')}` },
    { name: "description", content: t('auth.register.subtitle') },
  ];
}
...
export default function Register() {
  ...
  const { register, user, isLoading, refreshSession } = useAuth();
  const navigate = useNavigate();
  const location = useLocation();
  const { t } = useTranslation("common");
  const requestedDestination = new URLSearchParams(location.search).get('returnTo');
  const from = requestedDestination?.startsWith('/') ? requestedDestination : '/';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage = err.code ? t(`auth.errors.${err.code}`, err.message) : (err.message || 'Failed to register');
    ...
    <div className="min-h-screen flex items-center justify-center bg-background py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8">
        ...
        <div className="text-center">
          <Link to={`/login?returnTo=${encodeURIComponent(from)}`} className="font-medium text-primary hover:text-accent">
